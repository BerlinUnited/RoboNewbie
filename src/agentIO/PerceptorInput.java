 ...
 * @version 1.1 Changed by hdb 26.2.2014 (extensions for game state)
 * 
 ...
 * agent class" below). This class provides just immutable objects as return
 * values. This ensures, that other classes can not falsify the input from the
 * server. The getters for the vision perceptor values (getBall, getGoalPost,
 * getFlag, getPlayerPositions, getLines) should not be used directly, use class
 * localFieldView.LocalFieldView for that.
 *
 * Required context in the agent class: PerceptorInput has to be updated in
 * every server cycle and it is essential for keeping the agent program
 * synchronized with the server. So in the sense()-method of the agent class the
 * PerceptorInput-method update() must be the first command. See
 * Agent_BasicStructure, Agent_SimpleWalkToBall and Agent_SimpleSoccer for
 * examples of correct integration in Agent_-classes.
 *
 * The code of this class is partially copied from magmaOffenburg. \n (download
 * source of the magmaOffenburg-project: \n
 * http://robocup.fh-offenburg.de/html/downloads.htm, (14.1.2012) \n Used for
 * this class: src\magma\agent\perception\impl\ServerMessageParser.java).
 */
public class PerceptorInput {
    ...
    /**
     * Returns the raw coordinates of a goal post relative to the vision
     * perceptor ("camera").
     *
    ...
    /**
     * Returns the value of the force resistance perceptor in the left foot of
     * the robot.
     *
     * If the left force resistance perceptor detected a collision during this
     * server cycle, this method returns its data. If the sensor did not detect
     * any collision, the return is null. For more Information see class
    ...
    /**
     * Returns the value of the force resistance perceptor in the right foot of
     * the robot.
     *
     ...
     *
     * If other robots have sent messages, this method returns the messages. The
     * list contains just messages sent by other robots then the agents own, so
     * there can be at most two elements in this list: - one sent by any robot
     * of the agents own team, but not by itself - one sent by any robot of the
     * other team.
     *
    ...
    /**
     * Heinrich:
     * @return GPS position of the robot's torso
     */
    public Vector3D getGPS() {
        return gps;
    }

    /**
     * Returns the raw coordinates of all detected players relative to the
     * vision perceptor ("camera").
     *
     * This value is not accessible in every server cycle, see for details:
     * http://simspark.sourceforge.net/wiki/index.php/Perceptors#Vision_Perceptors
     *
     ...
     *
     * The simulated gyrometer is located in the torso of the robot and the
     * values are given in a right handed coordinate system: x-axis points to
     * the robots right, y in front of the robot and z points up.
     *
     ...
     *
     * The simulated accelerometer is located in the torso of the robot and the
     * values are given in a right handed coordinate system: x-axis points to
     * the robots right, y in front of the robot and z points up.
     *
     ...
     *
     * @param i Index of the joint as defined in class RobotConsts in package
     * util (like RobotConsts.RightFootPitch, RobotConsts.LeftArmRoll, ...) .
     ...
     *
     * @return Time elapsed since the start of the server until the actually
     * read server message.
     */
    ...
    /**
     * Returns a debug string with the values of all joint perceptors (in
     * degrees).
     *
    ...
    /**
     * Receives the server message and parses it to provide the perceptor
     * values.
     *
     * This method interacts almost directly (through a method of class
     * ServerCommunication) with the simulation server. It waits, until the next
     * server cycle begins, receives the new incoming server message and
     * initiates the parsing of the message. After this method is called, all
     * getter methods of this class provide the current values during the actual
     * server cycle. update() guarantees actual values, only if it is executed
     * in each cycle.
     *
     * About parsing internals: The server message is here transformed from a
     * character sequence into a symbol tree. Other specialized internal methods
     * read the informations from the tree leaves into the variables of
     * PerceptorInput.
     */
    public void update() {
     ...
     * message has differently structured leaves. This method decides, which
     * specialized parsing method will deal with the passed information type.
     *
    ...
    private void parseNode(SymbolNode node)
        ...
        switch (type) {
            ...
            case "GPS":
                ...
                break;
            default:
                // TODO: exception at unknown perceptor?
        }
    ...
    private void parseForceResistance(SymbolNode node)
      ...
      // read FRP-values, separated by left and right
     ...
     * Internal method for parsing the server message.
     *
     * Parse a symbol tree node into a hear perceptor value. "self"-messages are
     * omitted!
     *
     * @param node Symbol tree node
     * @throws PerceptorConversionException If the input string contains illegal
     ...
     * Internal method for parsing the server message.
     *
     * Parse a symbol tree node into a game state perceptor value. Only the
     * first server message after initialization contains unum: the player
     * number, and team: the side (left/right) of the team.
     *
     * @param node Symbol tree node
     * @throws PerceptorConversionException If the input string contains illegal
     * data which cannot be converted
    ...
    private void parseGameState(SymbolNode node)
        ...
        gameState = new GameStatePerceptor(time, playMode, side, id);
    ...
    private void parseGPS(SymbolNode node) 
            throws PerceptorConversionException
    {
        // expect the name
        for (int i = 1; i < node.children.length; i++) {
            SymbolNode child = (SymbolNode) node.children[i];
            String type = (String) child.children[0];
            
            switch(type)
            {
                case "n":
                    //System.out.println((String) child.children[1]);
                    // TODO: different torso names?
                    break;
                case "tf":
                    Object[] tf = child.children;
                    int k = 1;
                    // NOTE: rotation is not used yet
                    parseDouble(tf[k++]); parseDouble(tf[k++]); parseDouble(tf[k++]); double x = parseDouble(tf[k++]);
                    parseDouble(tf[k++]); parseDouble(tf[k++]); parseDouble(tf[k++]); double y = parseDouble(tf[k++]);
                    parseDouble(tf[k++]); parseDouble(tf[k++]); parseDouble(tf[k++]); double z = parseDouble(tf[k++]);
                    parseDouble(tf[k++]); parseDouble(tf[k++]); parseDouble(tf[k++]); parseDouble(tf[k++]);
